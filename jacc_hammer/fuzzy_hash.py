...
if __name__ == "__main__":
    ...
    overlap = 800
    glass = pd.read_csv(
        f"{project_dir}/data/processed/glass/orgs.csv",
        usecols=["id_organization", "organization_name"],
        nrows=nrows,
    )  # .pipe(preproc_names)
    names = (
        glass.head(overlap).organization_name.tolist(),
        glass.tail(overlap).organization_name.tolist(),
    )

    identifier = f"TEST_{uuid.uuid4().hex[:4]}"
    ...
    fout = f"/tmp/fuzzy_similarities_{identifier}"
    out = pipe(
        names,
        curry(match_names_stream, **match_config),
        # pd.concat,
        curry(stream_sim_chunks_to_hdf, fout=fout),
    )
    logger.debug(f"FINISHED! Output: {fout}")
